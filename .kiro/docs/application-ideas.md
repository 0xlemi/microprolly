...
## 4. Cool Desktop/Personal Applications

### Clipboard Manager with History

```
Keys:
  clip:{timestamp}      → JSON: {content, type, source_app, preview}
  idx:type:{mime_type}  → [timestamp1, timestamp2, ...]
  idx:app:{app_name}    → [timestamp1, timestamp2, ...]
  idx:day:{YYYY-MM-DD}  → [timestamp1, timestamp2, ...]
```

Why it's cool:
- "What did I copy 3 hours ago?" → time-travel through clipboard
- Search clipboard history by content or source app
- Sync clipboard between machines (future)
- Structural sharing: repeated copies of same content deduplicated
- Small data, high frequency - perfect for KV

---

### Screen Activity Recorder (Local "Recall")

```
Keys:
  frame:{timestamp}         → reference to screenshot blob
  desc:{timestamp}          → text description from vision model
  activity:{timestamp}      → JSON: {app, window_title, url, description}
  idx:app:{app_name}        → [timestamp1, timestamp2, ...]
  idx:hour:{YYYY-MM-DD-HH}  → [timestamp1, timestamp2, ...]
  blob:{hash}               → compressed screenshot (stored in CAS)
```

Why it's cool:
- Capture screenshots every 30-60 seconds
- Vision model generates descriptions: "User is editing Go code in VS Code"
- Query with LLM: "What was I working on this morning?"
- "Show me when I was researching Prolly Trees" → semantic search
- Privacy-first: all local, no cloud
- Linear history only - no merge needed (your activity is one timeline)
- Screenshots as blob references, descriptions in KV (small, searchable)

---

### Terminal Session Recorder with Semantic Search

```
Keys:
  cmd:{timestamp}           → JSON: {command, output, exit_code, duration, cwd, env}
  session:{id}              → JSON: {started, ended, shell, commands[]}
  idx:dir:{path}            → [timestamp1, timestamp2, ...]
  idx:exit:{code}           → [timestamp1, timestamp2, ...]  (find failures)
  idx:day:{YYYY-MM-DD}      → [timestamp1, timestamp2, ...]
```

Why it's cool:
- Record every command, its output, directory, environment
- "What command did I use to fix that Docker issue last month?"
- "Show me all failed commands from yesterday" → idx:exit:1
- LLM can understand context: "When was I debugging the API?"
- Like `asciinema` but queryable with AI understanding
- Structural sharing: similar outputs (like `ls` in same dir) deduplicated

---

### Local LLM Conversation Memory

```
Keys:
  conv:{id}                 → JSON: {model, started, summary}
  msg:{conv_id}:{sequence}  → JSON: {role, content, timestamp, tokens}
  idx:model:{model_name}    → [conv_id1, conv_id2, ...]
  idx:topic:{topic}         → [conv_id1, conv_id2, ...]  (auto-tagged by LLM)
  idx:day:{YYYY-MM-DD}      → [conv_id1, conv_id2, ...]
  summary:{conv_id}         → text summary (generated by LLM)
```

Why it's cool:
- Store all conversations with local LLMs (Ollama, llama.cpp)
- Build context across sessions: "Remember when we discussed Prolly Trees?"
- LLM can query its own history to maintain long-term memory
- "What did we decide about the API design?" → search past conversations
- Auto-generate topic tags and summaries
- Diff between conversation branches if you retry prompts

---

### Browser Session Time Machine

```
Keys:
  tab:{session}:{tab_id}    → JSON: {url, title, scroll_pos, form_data, opened_at}
  session:{timestamp}       → JSON: {tabs[], focused_tab, window_bounds}
  reading:{url_hash}        → JSON: {progress, highlights[], notes, last_visited}
  idx:domain:{domain}       → [url_hash1, url_hash2, ...]
  idx:day:{YYYY-MM-DD}      → [session1, session2, ...]
```

Why it's cool:
- Track tabs, scroll positions, form inputs, reading progress
- "Restore my browser state from yesterday morning"
- "What articles was I reading about Rust last week?"
- Not just history - the actual *state* of your browsing session
- Resume reading exactly where you left off
- Form data recovery: "What did I type in that form before the crash?"
